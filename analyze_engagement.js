...
const { RELAYS, YEAR } = require("./config");
...
function analyzeEngagement() {
  ...
  const engagementEvents = JSON.parse(
  ...
  const outPath = path.join(baseDir, "nostr_viral_insights.json");
  fs.writeFileSync(outPath, JSON.stringify(viralInsights, null, 2), "utf-8");
  console.log(`Viral insights generated to ${outPath}.`);
}
